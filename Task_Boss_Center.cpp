...
namespace  Boss_Center
{
	...
	bool  Object::Initialize()
	{
		...
		this->speed_chase_def = 0.05f;						//通常時の追従速度
		this->speed_chase_ref = 0.1f;						//反射被弾時の追従速度
	...
	void Object::Think()
	{
		...
		case Lose:				//死亡
			...
			if (head->state == Hiding_Under &&
				head->Get_Defeat_Parts() == DEFEAT_PARTS) { nm = Stand; }
			break;
		}
		//死亡処理
		switch (this->state)
	...
	void Object::Move()
	{
		...
		//基準となるタスクに追従する
		auto upper_middle = 
			ge->GetTask_One_GN<Boss_Upper_Middle::Object>(Boss_Upper_Middle::defGroupName,Boss_Upper_Middle::defName);
		//存在するか確認
		...
		this->pos.y = upper_middle->pos.y + float(this->hitBase.h);
