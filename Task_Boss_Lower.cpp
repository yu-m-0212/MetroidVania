...
namespace  Boss_Lower
{
	...
	bool  Object::Initialize()
	{
		...
		this->speed_chase_def = 0.025f;						//通常時の追従速度
		this->speed_chase_ref = 0.05f;						//反射被弾時の追従速度
	...
	void Object::Move()
	{
		...
		//基準となるタスクに追従する
		auto center = 
			ge->GetTask_One_GN<Boss_Center::Object>(Boss_Center::defGroupName,Boss_Center::defName);
		//存在するか確認
		if (nullptr == center) { return; }
		//反射された弾を被弾しているか調べる
		auto head =
			ge->GetTask_One_GN<Boss_Head::Object>(Boss_Head::defGroupName, Boss_Head::defName);
		...
		//目標に向かって移動する
		this->pos.x += this->boss->Chase_Target(this->pos.x, center->pos.x, this->speed_chase);
		this->pos.y = center->pos.y + float(this->hitBase.h);
